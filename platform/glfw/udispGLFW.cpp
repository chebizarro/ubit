/************************************************************************
 *
 *  udispGLFW.cpp: GLFW implementation
...
UCursor UCursor::pointer(GLFW_CURSOR_LEFT_ARROW, UCONST);
UCursor UCursor::crosshair(GLFW_CURSOR_CROSSHAIR, UCONST);
UCursor UCursor::text(GLFW_CURSOR_TEXT, UCONST);
UCursor UCursor::hand(GLFW_CURSOR_INFO, UCONST);
UCursor UCursor::pencil(GLFW_CURSOR_SPRAY, UCONST);  // ???GLFW_CURSOR_LEFT_ARROW
UCursor UCursor::wait(GLFW_CURSOR_WAIT, UCONST);
UCursor UCursor::question(GLFW_CURSOR_HELP, UCONST);
UCursor UCursor::hresize(GLFW_CURSOR_LEFT_RIGHT, UCONST);
UCursor UCursor::vresize(GLFW_CURSOR_UP_DOWN, UCONST);
UCursor UCursor::move(GLFW_CURSOR_INFO, UCONST);  // same sa hand
UCursor UCursor::dnd(GLFW_CURSOR_CYCLE, UCONST);  // ???
//UCursor UCursor::plus(GDK_PLUS, UCONST);
  
// ==================================================== [Ubit Toolkit] =========

UDispGLFW::UDispGLFW(const UStr& _dname) : UDisp(_dname)
{
  ...
  int display_modes = GLFW_RGBA | GLFW_DOUBLE | GLFW_ALPHA;
  if (depth_size > 0) display_modes |= GLFW_DEPTH;
  if (stencil_size > 0) display_modes |= GLFW_STENCIL;
  //if (accum_size > 0) display_modes |= GLFW_ACCUM;
  ...
  screen_width = glutGet(GLFW_SCREEN_WIDTH);
  screen_height = glutGet(GLFW_SCREEN_HEIGHT);
  screen_width_mm = glutGet(GLFW_SCREEN_WIDTH_MM);
  screen_height_mm = glutGet(GLFW_SCREEN_HEIGHT_MM);
  
  if (screen_width_mm == 0 || screen_height_mm == 0) {
    UAppli::warning("UDispGLFW","the resolution of the diplay is unknow (display %s)",dname);
  ...
  //  UAppli::error("UDispGLFW","could not create Rendering Context on display '%s'",dname);
  //  return;
  //}
  ...
  //default_rc->setDest(mainframe, 0, 0);  n'exsite pas sous GLFW!
  is_opened = true;
}
  
UDispGLFW::~UDispGLFW() {
  cerr << "UDispGLFW::~UDispGLFW() " <<endl;
  // if (sys_disp) ???CloseDisplay(sys_disp);
}

// ==================================================== [Ubit Toolkit] =========
  
unsigned long UDispGLFW::createColorPixel(const URgba& rgba) {
  return 0;
}

UHardwinGLFW* UDispGLFW::createWinImpl(UWin* win) {
  return new UHardwinGLFW(this, win);
}

UCursorImpl* UDispGLFW::createCursorImpl(int cursor_type) {
  // cas cursor_type == 0 ???
...
void UDispGLFW::deleteCursorImpl(UCursorImpl* ci) {
  delete ci;
}

// =============================================================================

void UDispGLFW::setPointerPos(const UPoint&) {
  UAppli::error("UDisp::setPointerPos","Not available with GLFW");
}

UPoint UDispGLFW::getPointerPos() const {
  UAppli::error("UDisp::getPointerPos","Not available with GLFW");
  return UPoint(0,0);
}

int UDispGLFW::getPointerState() const {
  UAppli::error("UDisp::getPointerState","Not available with GLFW");
  return 0;
}

bool UDispGLFW::grabPointer(const UCursor* c) {
  //UAppli::error("UDisp::grabPointer","Not available with GLFW");
  return false;
}

void UDispGLFW::ungrabPointer() {
  //UAppli::error("UDisp::ungrabPointer","Not available with GLFW");
}

bool UDispGLFW::pickWindow(int& x_in_win, int& y_in_win, UHardwinImpl* window,
                           UCursor* cursor, UCall* callback) {
  UAppli::error("UDisp::pickWindow","This function is not available when using GLFW");
  return false;
}

// ==================================================== [Ubit Toolkit] =========

void UDispGLFW::startAppli() {}
void UDispGLFW::quitAppli() {}
...
void UDispGLFW::startLoop(bool main) {
  try {
    glutMainLoop();
  }
  catch(UGlutQuit& e) {
...
void UDispGLFW::quitLoop(bool main) {
...
namespace glut {
  ...
  static UDispGLFW* _disp = null;   // only one display !!
  ...
  static void winMouseCB(int btn, int up_down, int x, int y) {
    ...
    _win_pos_x = glutGet(GLFW_WINDOW_X);
    _win_pos_y = glutGet(GLFW_WINDOW_Y);
    UPoint scr_pos(x + _win_pos_x, y + _win_pos_y);
    UPoint win_pos(x, y);
    //cerr << "winMouseCB " << _win_pos_x<<" "<<_win_pos_y<<" / "<<scr_pos<<endl;
    
    if (up_down == GLFW_DOWN) {
      switch (btn) {
        case GLFW_LEFT_BUTTON: 
  ...
  static void winEntryCB(int in_out) {
    ...
    if (in_out == GLFW_ENTERED) f->winEnter(winview, time);
    else if (in_out == GLFW_LEFT) f->winLeave(winview, time);
    UAppli::impl.processPendingRequests();
  }
  
  static void winDisplayCB() {    
    if (glutLayerGet(GLFW_NORMAL_DAMAGED)) { // glutDisplayCB() caused by window system
      UWin* win = _disp->retrieveWin(glutGetWindow());
      if (!win) return;
  ...
  static void winVisibilityCB(int state) {
    ...
    // state: GLFW_NOT_VISIBLE or GLFW_VISIBLE
  }
...
void UDispGLFW::initWinCallbacks(int sys_win) {
  glut::_disp = this;  // static var used by callback functions

  if (sys_win <= 0) {
    UAppli::error("UDispGLFW::initWinCallbacks","Wrong window ID: %d", sys_win);
    return; 
  }
